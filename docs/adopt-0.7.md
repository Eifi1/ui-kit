...
| gear/control charts | nothing to do | on the kit's `SeriesChart` since the 0.5 swap (done 2026-09-25): ticks are now round (1/2/5 × 10ⁿ) and missing points show "—" in the tooltip. `PAGE_CHART`, the grid-ink wrapper and `FacingRow` / `FacingHeadings` stay local. |
